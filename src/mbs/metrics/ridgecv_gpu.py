...
if __name__ == "__main__":
    # Simple test case to verify against sklearn
    from sklearn.linear_model import RidgeCV
    from sklearn.datasets import make_regression

    X_np, y_np = make_regression(n_samples=100, n_features=20, noise=0.1, random_state=42)
    alphas = [0.1, 1.0, 10.0]

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_skl = RidgeCV(alphas=alphas, store_cv_results=True).fit(X_np, y_np)
    model_torch = RidgeGCVTorch(alphas=alphas, store_cv_results=True, device=device).fit(X_np, y_np)

    print("Sklearn alpha:", model_skl.alpha_)
    print("Torch alpha  :", model_torch.alpha_)

    # print("Sklearn coef:", model_skl.coef_)
    # print("Torch coef:", model_torch.coef_)

    # print("Sklearn intercept:", model_skl.intercept_)
    # print("Torch intercept:", model_torch.intercept_)

    print("Sklearn best score   :", model_skl.best_score_)
    print("Torch best score     :", model_torch.best_score_)
    print("Best score difference:", np.abs(model_skl.best_score_ - model_torch.best_score_))

    # print("Sklearn cv results shape:", model_skl.cv_results_.shape)
    # print("Torch cv results shape:", model_torch.cv_results_.shape)