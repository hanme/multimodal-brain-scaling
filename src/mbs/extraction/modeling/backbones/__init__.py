...
class BackboneWrapperHF(nn.Module):
    """Wrapper class for the backbone model that preserves all its layers, 
        enabling the extraction of a complete computational graph with PyTorch hooks."""
    def __init__(self, backbone):
        super(BackboneWrapperHF, self).__init__()
        self.backbone = backbone
...
def create_backbone(**model_config) -> nn.Module:
    ...
    if backbone_source == "timm":
        ...
        model, transform = load_model_timm(**model_config)
    elif backbone_source == "spvvs":
        from .scaling_models import load_model_spvvs
        model, transform = load_model_spvvs(**model_config)
