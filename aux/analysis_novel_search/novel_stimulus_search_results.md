...
> 903 unordered pairs × {regular, counter} = **1,806 direction-instances** in Phase 1; the
...
> **Design.** 43 frequencies: a uniform ladder from 200 Hz in exact 1.500-semitone steps, to
> 7611 Hz. Semitone spacing keeps every step the same perceptual size; 12/1.5 = 8 exactly, so
> eight steps is an octave and 200/400/800/1600/3200/6400 are grid points. The top overshoots the
> nominal 7500 by one step because 62.746/1.5 is not an integer — 7611 Hz is still under the 8 kHz
> Nyquist, and every step stays identical. → 903 unordered pairs. Both directions of each
> pair are synthesized by the existing counterbalancing, so the 903 rows cover all **1,806
...
The phase sizes are fixed by design (903 pairs, then the top 145); the budgets are what they are
measured against, not an input to the selection.

| | dirs | clips/dir | budget CHF | predicted CHF | **actual CHF** |
|---|---|---|---|---|---|
| Phase 1 | 1,806 | 2 | 110 | ≈151 | ‹TBD› |
| Phase 2 | 290 | 14 (new only) | 220 | ≈160 | ‹TBD› |
| **total** | | | 330 | ≈311 | **‹TBD›** |

Reported separately, **not** charged to the budget: in-silico MMN (~3.5 CHF Phase 1, ~0.6 CHF
Phase 2, both with `--save_plots false`), audio synthesis, ranking, plots.

The "predicted" column is the measured gate rate, not the literature per-clip table — every
model beat its literature prediction on jed (whisper-tiny at 0.38x over a full run). Phase 1
exceeds its own 110 sub-budget on every reading, while the 330 total holds; the 110/220 split is
treated as a soft allocation. The Phase-2 pair count does **not** move in response — it is a flat
145 by design. See §17.4 of the repository overview.
...
**Table 2. Direction-instances by n_agree tier (Phase 1, /1806).** ‹TBD›
...
![n_agree over the 43×43 frequency grid; standard frequency on the vertical axis, deviant frequency on the horizontal, single-hue sequential ramp from light (0 models agree) to dark (all 6). The excluded diagonal is grey.](plots/novel_n_agree_heatmap.png)
...
> direction-instances. Phase-1 ranks are positions in the full 1,806-instance list, so they are
