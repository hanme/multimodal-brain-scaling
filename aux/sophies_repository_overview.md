...
**Last updated:** 2026-06-20 (doc pass: consistently label the in-use MMN stimulus design as **Definition 2** — literature classic-oddball, frequency-deviant — and flag it as provisional; see §1.6, §1.7, §5, §13.8, §16.1–§16.2)

**Changelog pointer:** the most recent work touches the **model→EEG mapping** (§4) and the **layer-selection CV** — the committed per-model layers in §1.5 were finalised on 2026-06-18 after switching from leaky random folds to group-by-part CV. See §4.1–§4.2 and Open Questions §13.1. The MMN-design terminology (Definition 1 vs 2) was straightened out on 2026-06-20 — see §16.1/§16.2.
...
- **MMN read-out:** time 0 = final/eliciting-tone onset; MMN = `deviant_mean − standard`, baseline-corrected on the pre-onset window, negative peak in the 100–250 ms (parcels) / 100–240 ms (electrodes) band. Design in use is **Definition 2 (literature classic-oddball, frequency-deviant)** — the deviant train's final tone differs in frequency from the standard's (§5, §16.1). **Caveat:** the current 10-method Definition 2 stimulus set is provisional, not yet fully vetted against the literature — see §16.2.
...
- ✅ The 8 placeholder MMN frequency pairs (`method_09`, `method_12(+_counter)`, old `method_37/44/55`) have been **replaced** with the 10-pair literature classic-oddball **Definition 2** set (§5, §16.1). `outputs/mmn_stimuli`/`data/audio_stimuli` still need populating/verifying on the cluster (Part B of the handover runbook) before features can be (re-)extracted for them.
- ⚠️ **This Definition 2 stimulus set is provisional** — chosen to get the pipeline running end-to-end, not yet fully vetted against the literature; 1–2 methods may be swapped in/out later. See §16.2 for the caveat and current inclusion criteria.
...
The 8 shipped `method_*` pairs described a now-superseded identity-MMN design (Definition 1, discontinued) and are gone. `METHODS` in `insilico_mmn.py` now holds the 10-pair literature classic-oddball **Definition 2** set (`data/metadata/literature_frequency_intensity_duration_metadata.csv`), already present as `data/audio_stimuli/`, but it is **provisional, not yet fully vetted** (§16.2). See §16.1 for why this is a different design from the old `method_37/44/55`, not a stale copy of it.
