...
**Last updated:** 2026-06-18 (matches latest commit `3394c24`, 2026-06-18: "Non-overlapping group-by-part CV for D2 mapping; commit chosen layers")
...
**Where it runs.** Locally for light work, but the real pipeline runs on the **EPFL SCITAS cluster** (`jed` = CPU partition, `kuma` = GPU partition `l40s`). Cluster repo root in the handover docs:
`/work/upschrimpf1/mehrer/code/20260601_multimodal_brain_scaling_schizophrenia/multimodal-brain-scaling`.
A second feature tree referenced by several scripts is Sophie's account:
`/work/upschrimpf1/sigfstea/multimodal-brain-scaling/outputs/features`.
...
- **MMN read-out:** time 0 = final/eliciting-tone onset; MMN = `deviant_mean − standard`, baseline-corrected on the pre-onset window, negative peak in the 100–250 ms (parcels) / 100–240 ms (electrodes) band. Design is **Definition 2 (physically controlled)** — the final tone is identical in standard and deviant.
...
- ⚠️ The 8 MMN frequency pairs currently shipped (`method_09`, `method_12(+_counter)`, `method_37`, `method_44(+_counter)`, `method_55(+_counter)`) are **placeholders**, to be replaced with vetted Definition-2 pairs (`aux/mmn_screening_plan.md`).
...
> **Important:** the function is named `load_split_targets` here. Three scripts still import the old name `load_split_parcels` — see Open Questions §13.2.

---

## 5. In-silico MMN

Drive the model→EEG mapping with designed MMN tone stimuli and read `deviant − standard` time-locked to the final tone. MMN stimulus dirs are `outputs/mmn_stimuli/<method>` with features at `outputs/features/mmn-<method>-delta-t` (built by `scripts/slurm_mmn_extract.sh`, §8). Method registry (`METHODS` in `insilico_mmn.py`): `method_37, method_12, method_44, method_09, method_55` + `_counter` variants.

### 5.1 `scripts/insilico_mmn.py`

**Purpose:** **Method A in-silico MMN at parcel level.** Fit a FIR mTRF on the training EEG (default D2) at one layer with electrodes aggregated into NC-floored parcels, apply the single mapping to each MMN method's delta-T features, time-lock to the final-tone onset, baseline-correct, compute `deviant_mean − standard`, plot a parcel grid, and write raw prediction arrays. Optionally also scores held-out test r (`--eval_heldout`).
...
**Key parameters (selected):** `--train_features`/`--train_neural` (aliases `--broderick_features`/`--broderick_neural`; default D2 whisper-small feats + `surprisal_30s.h5`), `--mmn_features_root` (`outputs/features`), `--stimuli_root` (`outputs/mmn_stimuli`), `--layer` (`blocks.3`), `--methods` (`all`), `--nc_r_threshold` (0.2), `--highpass_hz` (0.5), `--lag_max_ms` (**500** — handover recommends 800 to match the sweep), `--n_train_time_samples` (120), `--eval_heldout` (True), `--alpha_log_min/max`/`--alpha_n` (1/7/25), `--win_pre_ms`/`--win_post_ms` (150/500), `--out_dir` (`outputs/figures/insilico_mmn`), `--data_dir` (`outputs/insilico_mmn_predictions`).

**Produced outputs:**
- Figures `outputs/figures/insilico_mmn/insilico_mmn__<method>__<layer>.png`.
...
**Step-by-step:** build parcels (NC floor) → `fit_mapping` once for the layer → for each method: load delta-T feats, predict per stimulus (`predict_timecourse`), detect final-tone onset (`detect_final_tone_onset_s`), time-lock, baseline-correct, plot + store.

**Key functions:** `fit_mapping`, `evaluate_heldout`, `predict_timecourse`, `analyze_method`, `plot_method`, `detect_final_tone_onset_s`, `main`. Imports `build_parcels`/`load_split_targets` from §4.8 and lag helpers from §4.1.
...
**Purpose:** Electrode-level in-silico MMN with a **topographic** 10-20 montage plot and an **automatic MMN verdict** (mean `deviant − standard` in 100–240 ms over a fronto-central ROI; negative beyond `--mmn_thresh` → "MMN present"). Reuses `fit_mapping`/`analyze_method` from §5.1 with electrodes as singleton targets.
...
**Key parameters (extra vs §5.1):** `--layer` (default `blocks.10`), `--mmn_lo_ms`/`--mmn_hi_ms` (100/240), `--mmn_thresh` (0.0), `--win_post_ms` (400), `--out_dir` (`outputs/figures/insilico_mmn_electrodes`); FC ROI = `Fz,FCz,Cz,FC1,FC2,F1,F2` (intersected with surviving electrodes).

**Produced outputs:** figures `insilico_mmn_electrodes__<method>__<layer>.png`; predictions `electrode_predictions__<layer>.h5` with per-method `roi_mmn_amp`/`mmn_present` attrs; prints a per-method MMN screen summary (`N/total pairs show an MMN`).

**Key functions:** `mmn_metric`, `plot_topo`, `main`; `build_electrodes`/`montage_pos` from §4.8.
...
python scripts/insilico_mmn_attn.py \
  ...
  --neural outputs/neural_data/surprisal_30s.h5 \
  --out_dir outputs/figures/insilico_mmn_small \
  --data_dir outputs/insilico_mmn_predictions_small
```
Cluster driver: `scripts/slurm_insilico_mmn_attn.sh` (§8).

**Key parameters:** `--checkpoint` (required), `--mmn_features_root` (required), `--method` (`method_09`), `--features_dir`/`--neural` (required, for fit-quality), `--win_pre_ms`/`--win_post_ms` (150/500), `--window_idx` (-1), `--device` (`cpu`), `--out_dir`/`--data_dir`, `--tag` (`attn`).
...
**Key functions:** `analyze_method_attn`, `fit_quality_figure`, `parcels_from_ckpt`, `main`. Uses `load_probe_checkpoint`/`predict_timecourse`/`predictions_to_units` from `attn_probe/checkpoint.py`; reuses `METHODS`/`plot_method`/`detect_final_tone_onset_s` from §5.1.
...
> **⚠ Blocked at import time:** §5.3 imports `load_split_parcels` from `insilico_mmn`, which no longer exists (renamed to `load_split_targets` in §4.8). See §13.2.
...
**Outputs:** stdout per-parcel table + optional JSON. **⚠ Imports `load_split_parcels` from `insilico_mmn` → broken (§13.2).**
...
**Outputs:** one PNG at `--out`. **⚠ Imports `load_split_parcels` from `insilico_mmn` → broken (§13.2).**
...
**Outputs:** one PNG. Calls `build_parcels`/`fit_mapping` from §5.1 (uses `eval_heldout`, not `load_split_parcels` — so this one imports successfully).
...
| `slurm_mmn_extract.sh` | same | MMN WAVs (`MMN_METHOD` default `method_09`); 16-task array; `--window_stride 30` |
...
6. **Read the MMN** from `predictions__<layer>*.h5` (deviant_mean − standard, 100–250 ms band).
...
| `score_mtrf_fitquality.py` (§6.1) | imports §5.1 |
| `plot_eeg_mapping.py` (§6.2) | §4.6/§4.7 JSON |
| `plot_fit_quality.py` (§6.3) | imports §5.1 |
...
ls outputs/neural_data/surprisal_30s.h5 # D2 must exist (see §13.4)
```

**Stage A — EEG formatting (D1; D2/D3 supplied):**
```bash
python -m mbs.data_prep.format_eeg_hdf5 \
  --bids_root /work/upschrimpf1/mehrer/datasets/Broderick_2018_EEG_The_old_man_and_the_sea \
  --output_path outputs/neural_data/broderick2018_30s.h5 \
...
**Stage D — MMN stimulus features:**
```bash
MMN_METHOD=method_09 MODEL_ID=whisper-small sbatch scripts/slurm_mmn_extract.sh
```

**Stage E — in-silico MMN (use committed §1.5 layers, `--lag_max_ms 800`):**
```bash
...
**Stage F — read the MMN** (`outputs/insilico_mmn_predictions*/predictions__<layer>*.h5`): `mmn = deviant_mean − standard`, baseline-correct on the pre-onset window, negative peak in 100–250 ms (parcels) / 100–240 ms (electrodes). Snippet in `aux/XX_handover_for_Sophie.md` §2.
