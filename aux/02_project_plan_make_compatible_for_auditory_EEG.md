...
## Phase 4b — Temporal evaluation (whisper-base × ds004408) 🔄 In progress (2026-06-02)
...
- SLURM: `scripts/slurm_extract_delta_t.sh`
...
**Current status:**
- [x] `extract_features_delta_t.py` written and tested (compute node pilot: 3 stim × 10 bins, ~20s)
- [x] SLURM pilot submitted (job 54867238, 3 stim × 1500 bins, ~40 min)
- [ ] Full SLURM run (20 tasks × 16 stim) — pending pilot success
- [ ] Temporal evaluation — pending features

**Full commands:** see `XX_handover_for_Sophie.md` Section 5b.

**What to look for (ds004408, naturalistic speech):**
- Scores meaningfully above zero across most time bins at Fz, T7, T8 (not a sharp peak — this
  is naturalistic speech, not an oddball paradigm)
- Layer ranking consistent with Phase 4a (blocks.2–4 best at Fz)
- Sharp 50–300ms peak is the success criterion for Phase 6 (MMN), not here

Success criterion (ds004408): mean prediction score > 0 across most time bins for Fz/T7/T8.

---

## Phase 5 — Scale to all 9 models

Repeat Phases 4a + 4b for all models in Sophie's pipeline.
...
## Phase 6 — MMN EEG dataset integration (TBD)
...
- [ ] Re-run Phase 4b temporal evaluation with Sophie's exact MMN stimuli:
  - Key output: prediction score at Fz as a function of time (0–500ms) and model layer
  - Does the score peak at ~100–200ms? That would confirm the model captures MMN-relevant processing
- [ ] Compare healthy vs. schizophrenia temporal prediction profiles:
  - Is the 100–200ms peak weaker or delayed in patients?
  - Does model-to-brain alignment at the MMN latency differ between groups?
- [ ] Cross-reference best-predicting layer with Sophie's MMN unit selection:
  - Do the units selected by Sophie's pipeline come disproportionately from the
    best-predicting layer found here?
...
4. **FCz noise ceiling is 0% (unexpected):** In the verified run (2026-06-01), FCz NC averaged
   to 0.0% across all 1500 time bins, while the classical MMN frontal electrodes Fz (50.4%) and
   F3/F4 (~20%) were fine. FCz is a standard MMN electrode in the literature (e.g. Näätänen et al.),
   so this is worth investigating before drawing conclusions about that ROI.
   Possible causes:
   - The `biosemi128` MNE montage's nearest-neighbour mapping may assign FCz to a BioSemi channel
     (A/B/C/D naming) that is poorly positioned or noisy in this dataset.
   - For continuous naturalistic speech (audiobook), the auditory response is dominated by temporal
     and parietal electrodes; the frontocentral vertex may simply have too weak a sustained response
     for cross-subject NC to emerge when averaged across 1500 bins.
   **Action:** inspect raw data at the BioSemi channel assigned to FCz; check the actual coordinates
   via `mne.channels.make_standard_montage("biosemi128")` and confirm which channel maps to FCz.
